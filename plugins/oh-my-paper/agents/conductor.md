...
检查 `.pipeline/memory/settings.md` 中是否设置 `AUTO_DECISION=true`：

```bash
cat .pipeline/memory/settings.md 2>/dev/null | grep AUTO_DECISION
```

如果 `AUTO_DECISION=true`，Conductor 将自动执行决策循环，无需等待人工确认。

### 决策规则（按优先级）

| 条件 | 决策 | 自动执行 |
|------|------|---------|
| 结果达标（≥ successThreshold）+ 证据充分 + 无 CRITICAL 问题 | **PROCEED** | 继续下一阶段，更新 tasks.json 和 project_truth.md |
| 结果接近达标（≥ 80% threshold）+ 证据可修复 | **REFINE** | 返回上一步调整，版本+0.1 |
| 结果完全不达标（< 50% threshold）+ 假设被证伪 | **PIVOT** | 返回 A6 重新假设，版本+1.0 |

### 决策检查清单（自动执行）

在做出 PROCEED 决策前，必须验证以下所有项：

- [ ] **无 NaN/Inf**：实验日志中不存在 NaN 或 Inf 值
- [ ] **Trial count**：实验日志中 trial 数 ≥ 计划数
- [ ] **收敛检查**：loss 确实在下降或收敛
- [ ] **统计显著性**：如适用，t-test/ANOVA 已执行
- [ ] **假设一致性**：结果与假设方向一致
- [ ] **无 CRITICAL FABRICATION**：论文声称与实验日志一致

### 自动执行流程

当所有检查通过且决策为 PROCEED 时：

```
1. 更新 tasks.json:
   - 当前任务: in-progress → done
   - nextTask: 设为下一阶段任务
...
3. 自动推进到下一阶段:
   - 如果是 Mega 模式，更新 PROGRESS.md
   - 启动下一阶段的 execution_context.md
...
| 情况 | 原因 |
|------|------|
| 决策为 **PIVOT** | PIVOT 是重大变更，需要用户知情 |
| **CRITICAL** 问题发现 | 数据伪造或严重不一致 |
| 用户设置 `AUTO_DECISION=false` | 用户选择手动控制 |
| 决策为 **REFINE** 但无明确修复方向 | 需要用户决定如何调整 |
...
> ⚠️ **PIVOT 决策 - 需要确认**
>
> 实验结果完全不达标（< 50% threshold），假设被证伪。
>
> 建议：返回 A6 重新生成假设（版本 v2.0）
>
> 选项：
> - `确认 PIVOT`
> - `先尝试 REFINE`
...
> ⚠️ **CRITICAL FABRICATION 发现 - 需要确认**
>
> 论文声称 10 种数据集，实验日志只有 2 种。
>
> 这是严重的数据不一致问题。
>
> 选项：
