---
name: baibaiAIGC
description: '对中文或英文技术/学术文本进行降 AIGC 改写。支持两种模式：
1. **单段输入**：用户直接粘贴一段文字（≤850字），直接处理并返回结果 + 评分
2. **整篇文档**：严格按两轮顺序使用 prompts/baibaiAIGC1.md、prompts/baibaiAIGC2.md；英文模式只执行一轮
每次调用只执行一轮改写，依靠“降 AIGC 记录”跨对话串联轮次，并在最终根据 checklist.md 做检查和评分。当前项目不再使用第 3 轮。'
user-invocable: true
---
...
**单段输入（≤850字）**：直接读取当前轮 prompt 并改写，无需切分，直接返回结果 + 评分。
